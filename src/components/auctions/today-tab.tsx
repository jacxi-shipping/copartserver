...
export function TodayTab() {
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count
  }, [filterMake, filterBodyStyle, filterFuelType])
  ...
  const fetchAuctions = useCallback(
    async (p: number = 1) => {
      ...
      try {
        ...
        const res = await fetch(`/api/auctions/today?${params.toString()}`)
        if (res.ok) {
    ...
    [sortBy, filterMake, filterBodyStyle, filterFuelType, viewMode]
  )
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Today&apos;s Lots</h2>
          <p className="text-sm text-muted-foreground">{todayStr}</p>
        </div>
        <Badge className="border-emerald-300 bg-emerald-50 px-3 py-1 text-sm font-medium text-emerald-700 dark:border-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm font-medium text-muted-foreground">
              No lots scheduled for today
            </p>
            <p className="mt-1 text-xs text-muted-foreground/70">
              {activeFilterCount > 0
                ? 'Try adjusting your filters'
                : 'Check the Upcoming tab for future sale dates'}
            </p>
            {activeFilterCount > 0 && (
              ...
              <motion.div key={auction.id} {...fadeIn}>
                <div className="relative">
                  ...
                  <div className="cursor-pointer" onClick={() => handleVehicleClick(auction)}>
                    <div className="relative">
                      <VehicleCard auction={auction} showLiveBadge />
